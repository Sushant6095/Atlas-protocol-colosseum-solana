// /docs/playground — interactive API console.
//
// Catalog → request panel → response panel. Live request hits
// production reads via the @atlas/sdk client; replay mode pins the
// request to a given slot via the warehouse.
...
description: "Interactive request console for every Atlas REST endpoint."
---
# API playground

Pick an endpoint, fill the parameter, click Run. The console hits
production reads through the typed client — no mocks. Toggle replay
mode to pin the request to a historical slot via the warehouse.
`;
...
export default function PlaygroundPage(): JSX.Element {
  ...
  return (
    ...
    >
      <div className="not-prose grid grid-cols-12 gap-4">
        <Panel surface="raised" density="dense" className="col-span-12 md:col-span-3">
        ...
        <Panel surface="raised" density="dense" className="col-span-12 md:col-span-4">
          ...
          <p className="mt-3 text-[10px] text-[color:var(--color-ink-tertiary)]">
            {replayMode
              ? "replay mode pins the request to as_of_slot via the warehouse."
