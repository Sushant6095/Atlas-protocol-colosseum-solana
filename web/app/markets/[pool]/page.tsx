  ...
  if (!poolQ.data) {
    return (
      <main className="mx-auto max-w-6xl px-6 py-20 text-center">
        ...
        <button
          onClick={() => router.back()}
          className="inline-flex items-center gap-2 rounded-lg glass px-5 py-2.5"
        >
  ...
  return (
    <main>
      <section className="mx-auto max-w-6xl px-6 pt-8 pb-6">
        <Link
          href="/markets"
          className="inline-flex items-center gap-1.5 text-sm text-[color:var(--color-muted)] hover:text-ink-primary transition mb-6"
        >
        ...
        >
          <div className="md:col-span-2 space-y-2">
            ...
            <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight">{p.symbol}</h1>
            {p.poolMeta && (
      ...
      <section className="mx-auto max-w-6xl px-6 pb-8">
        <div className="glass rounded-lg p-6">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-1 p-1 rounded-lg border border-[color:var(--color-border)] bg-surface-base/30">
              {(["apy", "tvl"] as const).map((m) => (
                <button
                  key={m}
                  onClick={() => setMetric(m)}
                  className={`px-3 py-1.5 text-xs uppercase tracking-wider rounded-md transition ${
                    metric === m ? "bg-white/10 text-ink-primary" : "text-[color:var(--color-muted)]"
                  }`}
                >
                  {m === "apy" ? "APY" : "TVL"}
                </button>
          ...
          {chartQ.error && (
            <div className="h-80 flex items-center justify-center text-accent-danger text-sm">
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <aside>
            <div className="glass rounded-lg p-6 sticky top-28 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-display font-semibold">Route via Atlas</h3>
              ...
              <Link
                ...
                className="block w-full text-center py-3 rounded-lg bg-gradient-to-r from-[#7c5cff] to-[#29d3ff] text-ink-primary font-medium glow-accent hover:opacity-95 transition"
              >
                Open Atlas Vault
              </Link>
              ...
              {p.url && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-between rounded-lg border border-[color:var(--color-border)] px-3 py-3 text-sm hover:bg-white/5 transition"
                >
