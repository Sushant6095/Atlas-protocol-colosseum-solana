  ...
  if (!v) {
    return (
      <main className="mx-auto max-w-6xl px-6 py-20 text-center">
        <h1 className="text-2xl font-semibold mb-2">Vault not found</h1>
        <button onClick={() => router.push("/vaults")} className="rounded-xl glass px-5 py-2.5 inline-flex items-center gap-2">
  ...
  return (
    <main>
      <section className="mx-auto max-w-6xl px-6 pt-8 pb-2">
        <Link href="/vaults" className="inline-flex items-center gap-1.5 text-sm text-[color:var(--color-muted)] hover:text-white transition mb-6">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight">{v.asset} · {v.symbol}</h1>
            ...
            <div className="glass rounded-2xl p-1 inline-flex">
              {TABS.map((t) => (
                <button
                  key={t.k}
                  onClick={() => setTab(t.k)}
                  className={`relative px-4 py-2 rounded-xl text-sm transition inline-flex items-center gap-1.5 ${
                    tab === t.k ? "text-white" : "text-[color:var(--color-muted)] hover:text-white"
                  }`}
                >
                  {tab === t.k && (
                    <motion.span
                      layoutId="vault-tab"
                      className="absolute inset-0 rounded-xl bg-gradient-to-r from-[#7c5cff] to-[#29d3ff]"
          ...
          <aside>
            <div className="glass rounded-2xl p-6 sticky top-28">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold">Your deposits</h3>
                ...
                {(["deposit", "withdraw"] as const).map((m) => (
                  <button
                    ...
                    className={`relative flex-1 py-2 rounded-lg text-sm font-medium capitalize transition ${
                      depositMode === m ? "text-white" : "text-[color:var(--color-muted)]"
                    }`}
                  >
                    {depositMode === m && (
                      <motion.span
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="glass rounded-2xl p-4">
  ...
  return (
    <div className="glass rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <h3 className="font-semibold">30-Day APY · Performance · TVL</h3>
        <div className="flex gap-1 p-1 rounded-lg border border-[color:var(--color-border)] bg-black/30">
          {RANGES.map((r) => (
            <button
              key={r.k}
...
function InfoTab({ vault }: { vault: AtlasVaultMeta }) {
  return (
    <div className="glass rounded-2xl p-6 space-y-4">
      <h3 className="font-semibold">Vault info</h3>
...
function StrategiesTab({ vault }: { vault: AtlasVaultMeta }) {
  return (
    <div className="space-y-4">
      <div className="glass rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold">Strategy allocation</h3>
        ...
        <div className="flex h-3 rounded-full overflow-hidden mb-5">
          {vault.legs.map((leg, i) => (
            <div
              ...
              className={i > 0 ? "border-l border-black" : ""}
            />
          ))}
        </div>
  ...
  return (
    <div className="glass rounded-2xl p-6 space-y-4">
      ...
      <div className="border-t border-[color:var(--color-border)] pt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
        <a href={vault.docsUrl} className="flex items-center justify-between rounded-xl border border-[color:var(--color-border)] px-3 py-3 hover:bg-white/5 transition">
          <span>How proofs work</span> <ArrowUpRight className="h-4 w-4 text-[color:var(--color-accent)]" />
        </a>
        <a href={vault.apiUrl} className="flex items-center justify-between rounded-xl border border-[color:var(--color-border)] px-3 py-3 hover:bg-white/5 transition">
          <span>Vault API</span> <ArrowUpRight className="h-4 w-4 text-[color:var(--color-accent)]" />
        </a>
        <a href="/proofs" className="flex items-center justify-between rounded-xl border border-[color:var(--color-border)] px-3 py-3 hover:bg-white/5 transition">
          <span>Proof feed</span> <ArrowUpRight className="h-4 w-4 text-[color:var(--color-accent)]" />
        </a>
        <a href="https://github.com" target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-xl border border-[color:var(--color-border)] px-3 py-3 hover:bg-white/5 transition">
...
function CopyRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-[color:var(--color-border)] bg-black/30 p-3">
      <div className="min-w-0">
        <div className="text-xs text-[color:var(--color-muted)]">{label}</div>
        <div className="font-mono text-xs truncate">{value}</div>
      </div>
      <button onClick={copy} className="ml-3 text-[color:var(--color-muted)] hover:text-white">
